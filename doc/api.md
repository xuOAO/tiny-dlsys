...
`Device` 方法直接操作后端原始数据（CPU 为 `numpy.ndarray`，CUDA 为 GPU buffer），
而不是上层 `NDArray` 对象。
...
class Device:
    ...
    def zeros(self, shape: tuple, dtype="float32") -> Any: ...
    def ones(self, shape: tuple, dtype="float32") -> Any: ...
    def randn(self, shape: tuple, dtype="float32") -> Any: ...
    def empty(self, shape: tuple, dtype="float32") -> Any: ...
...
class CUDADevice(Device):  # 当前桩实现：enabled() == False
    ...
